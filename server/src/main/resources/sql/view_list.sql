...
from (select cp.id              as post_id,
      ...
      where p.post_status = '정상'
        and cp.channel_post_status = '정상') p
         join
     (select c.id                as channel_id,
      ...
      from tbl_channel c
               join tbl_member m
                    on c.member_id = m.id) c on p.channel_id = c.channel_id;
...
from tbl_channel_post_report pr
         join tbl_report r on r.id = pr.id;
...
from (select cp.id              as id,
             ...
             p.updated_date     as updated_date
      from tbl_channel_post cp
               join tbl_post p on cp.id = p.id) p
         join (select m.id as member_id,
                      m.member_file_name,
                      m.member_file_path,
                      m.member_nickname,
                      c.id as channel_id,
                      ...
                      c.channel_file_name
               from tbl_member m
                        join tbl_channel c on m.id = c.member_id) m on p.member_id = m.member_id;
...
from tbl_member m
         ...
         join tbl_channel c
              on m.id = c.member_id
         join tbl_diary_score ds on d.diary_score = ds.id;
