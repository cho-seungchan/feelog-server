...
document.addEventListener("DOMContentLoaded", () => {
    document.body.addEventListener("click", (e) => {
        ...
        // 댓글 이미지 추가 버튼 클릭시
        if (e.target.closest(".joy-krt2ji")) {
            const input = document.createElement("input");
            input.type = "file";
            input.accept = "image/*";

            // 최대 1개 :: <ul class="MuiStack-root joy-1p6iz8l"> 에 data-count 추가해서 체크하는거로 수정
            input.addEventListener("change", (event) => {
    ...
    document.body.addEventListener("input", (e) => {
        ...
        if (e.target.classList.contains("joy-1qmqn4r")) {
            const textarea = document.querySelector(".joy-1qmqn4r");

            // 등록버튼 활성화 비활성화
            const button = document.querySelector(".joy-3oxsei");
            const hasDisabledClass = button.classList.contains("Mui-disabled"); // Mui-disabled 존재 여부 확인

            if (textarea.value.trim().length > 0) {
                // 글자가 있고 Mui-disabled 클래스가 있다면 제거
                if (hasDisabledClass) {
                    button.classList.remove("Mui-disabled");
                }
            } else {
                // 글자가 없고 Mui-disabled 클래스가 없다면 추가
                if (!hasDisabledClass) {
