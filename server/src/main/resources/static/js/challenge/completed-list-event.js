...
document.addEventListener("DOMContentLoaded", () => {
    ...
    // 완료 클릭 이벤트
    document.querySelectorAll(".complete-member-task").forEach(task => {
        task.addEventListener("click", e => {
            ...
            if (e.target.classList.contains("selected")) {
                ...
                // 도전 버튼 보이기
                e.target.closest(".button-conatiner").querySelector(".challenge-member-task").style.display = "block";
                cancelCompleteMemberTask(requestBody);
            } else {
                e.target.classList.add("selected");
                e.target.textContent = '완료취소';
                // 도전 버튼 숨기기
                e.target.closest(".button-conatiner").querySelector(".challenge-member-task").style.display = "none";
                completeMemberTask(requestBody);
            }
        });
    });

    document.querySelectorAll(".complete-common-task").forEach(task => {
        task.addEventListener("click", e => {
            ...
            if (e.target.classList.contains("selected")) {
                e.target.classList.remove("selected");
                e.target.textContent = '완료';
                cancelCompleteCommonTask(requestBody);
                // 도전 버튼 보이기
                e.target.closest(".button-conatiner").querySelector(".challenge-common-task").style.display = "block";
            } else {
                e.target.classList.add("selected");
                e.target.textContent = '완료취소';
                // 도전 버튼 숨기기
                e.target.closest(".button-conatiner").querySelector(".challenge-common-task").style.display = "none";
                completeCommonTask(requestBody);
            }
        });
    });
    // 완료 클릭 이벤트
